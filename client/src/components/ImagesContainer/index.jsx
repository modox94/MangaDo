  ...
  return (
    <div className={styles.container}>
      {translateMarkArr.map((mark) => {
        return (
          <Draggable onStop={(e, position) => { onControlledDragStop(e, position, mark._id) }} position={mark.position}>
            <div className={`${mark.visible ? styles.markTranslate : styles.disableMark}`}>
              <div>П</div>
            </div>
          </Draggable>
        )
      ...
      {decorMarkArr.map((mark) => {
        return (
          <Draggable onStop={(e, position) => { onControlledDragStop(e, position, mark._id) }} position={mark.position}>
            <div className={`${mark.visible ? styles.markDecor : styles.disableMark}`}>
              <div>О</div>
            </div>
          </Draggable>
        )
      ...
      {editMarkArr.map((mark) => {
        return (
          <Draggable onStop={(e, position) => { onControlledDragStop(e, position, mark._id) }} position={mark.position}>
            <div className={`${mark.visible ? styles.markEdit : styles.disableMark}`}>
              <div>Р</div>
            </div>
          </Draggable>
        )
